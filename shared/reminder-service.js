(() => {
  ...
  function normalizeReminder(item) {
    ...
    const timing = ["start", "after5", "before10", "before5", "custom"].includes(item.timing)
      ? item.timing
      : "start";
    const customMinutes = Math.max(1, Math.min(60, Math.floor(Number(item.customMinutes) || 10)));
    const privacy = item.privacy === "private" ? "private" : "normal";
    ...
    return {
      ...
      studentClassId: /^[1-7]$/.test(String(item.studentClassId || "")) ? String(item.studentClassId) : "",
  ...
  function getTriggerTime(reminder) {
    ...
    switch (reminder.timing) {
      case "after5":
        ...
        break;
      case "before10":
        // Kept for backward compatibility with reminders created before Custom timing was added.
        trigger = addMinutes(windowInfo.end, -10);
        break;
      case "custom":
        trigger = addMinutes(windowInfo.end, -Math.max(1, Math.min(60, Number(reminder.customMinutes) || 10)));
        break;
      case "before5":
  ...
  function timingLabel(timing, customMinutes = 10) {
    if (timing === "custom") {
      const minutes = Math.max(1, Math.min(60, Math.floor(Number(customMinutes) || 10)));
      return `${minutes} minute${minutes === 1 ? "" : "s"} before class ends`;
    }
  ...
  function addReminder(input) {
    ...
    const reminder = normalizeReminder({
      ...
      studentClassId: String(input?.studentClassId || ""),
    ...
    if (reminder.date === dateKey() && triggerInfo.trigger <= new Date()) {
      throw new Error(`${timingLabel(reminder.timing, reminder.customMinutes)} has already passed for ${triggerInfo.className} today.`);
    }
  ...
  function resolveStudentDisplay(reminder) {
    ...
    const classIds = [];
    if (reminder.studentClassId) classIds.push(String(reminder.studentClassId));
    if (reminder.classId && !classIds.includes(String(reminder.classId))) classIds.push(String(reminder.classId));

    try {
      const activeClasses = DashboardData.getClasses({ activeOnly: true }) || [];
      activeClasses.forEach(item => {
        if (!classIds.includes(String(item.id))) classIds.push(String(item.id));
      });
    } catch (_) {}

    for (const classId of classIds) {
      try {
        const student = DashboardData.getStudent(classId, reminder.studentId);
  ...
  function showPopup(reminder) {
    ...
    reveal.type = "button";
    reveal.className = "td-reminder-show";
    reveal.textContent = "Show";

    function renderPrivateState() {
      const contextBits = [
        triggerInfo?.className || `Period ${reminder.classId}`,
        revealed ? (studentName || "") : "",
        timingLabel(reminder.timing, reminder.customMinutes)
      ].filter(Boolean);

      head.innerHTML = `
        <div class="td-reminder-eyebrow">${isPrivate ? "Private Class Reminder" : "Class Reminder"}</div>
        <div class="td-reminder-title">${revealed ? (studentName ? escapeHtml(studentName) : "Reminder") : "Private Reminder"}</div>
      `;

      context.textContent = contextBits.join(" • ");
      message.textContent = revealed ? reminder.text : "Reminder details are hidden.";
      reveal.style.display = isPrivate && !revealed ? "inline-block" : "none";
    }
    ...
    actions.append(reveal, snooze, done);
  ...
  function clearCompleted() {
    const before = load();
    const remaining = before.filter(item => item.status !== "done");
    const cleared = before.length - remaining.length;
    if (cleared > 0) {
      save(remaining, { type: "completed-reminders-cleared", cleared });
    }
    return cleared;
  }
  ...
  function getReminders(options = {}) {
    ...
    return reminders.map(item => {
      ...
      return {
        ...
        timingLabel: timingLabel(item.timing, item.customMinutes),
