(() => {
  ...
  function normalizeData(raw) {
    ...
    for (let i = 1; i <= 7; i += 1) {
      ...
      const studentRecords = normalizeStudentRecords(
        ...
        classroom.mappings[id]?.students ?? []
      );
  ...
  function saveClassroomRoster(classId, course, students) {
    ...
    const studentRecords = normalizedStudents.map(student => ({
    ...
    // The name-only roster remains for backward compatibility, while modern
    // Dashboard utilities use stable studentRecords.
    data.classes[id].studentRecords = studentRecords;
    data.classes[id].students = normalizeStudents(
      studentRecords.map(student => student.name)
    );
  ...
  function markParticipationGradeSynced(
    ...
    assignment.lastSyncedAt = String(syncedAt || new Date().toISOString());

    return save(data, {
      type: "participation-grades-synced",
      classId: classKey,
      termId: termKey,
      syncedAt: assignment.lastSyncedAt
    });
  }
