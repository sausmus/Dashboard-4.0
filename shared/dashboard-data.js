(() => {
    ...
    if (!courseId) {
      ...
      data.classroom.mappings[id] = {
        ...
        lastRosterChange: sameCourse
          ? previous.lastRosterChange || { added: [], removed: [], renamed: [], initialImport: false, at: "" }
          : { added: [], removed: [], renamed: [], initialImport: false, at: "" },
  ...
  function saveClassroomRoster(classId, course, students) {
    ...
    const added = normalizedStudents
      .filter(student => !previousById.has(student.id))
      .map(student => ({ id: student.id, name: student.name }));
    const removed = previousStudents
    ...
    data.classroom.mappings[id] = {
      ...
      lastImportedAt: now,
      lastRosterChange: { added, removed, renamed, initialImport, at: now },
