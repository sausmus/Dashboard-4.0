(() => {
  ...
  function getTimelinessPolicy(termId = getActiveTimelinessTermId()) {
    ...
    return clone({
      ...
      graceLateSubmissions: term.graceLateSubmissions,
  ...
  function calculateTimelinessScore(lateCount, policy) {
    ...
    const deductions = Math.max(0, count - Number(p.graceLateSubmissions || 0)) * Number(p.deductionPerLateSubmission || 1);
